...
public class AutoCompleteBean implements Serializable
{
  ...
  {
    return CarConverter.cars.values().stream()
      .filter(car -> car.getName().startsWith(input))
      .toList();
  }
  ...
  {
    return CarConverter.cars.values().stream()
      .filter(car -> car.getName().contains(input))
      .toList();
  }

  public void handleSelect(SelectEvent event)
  {
    MessageUtil.addInfoMessage("selected.object", event.getObject());
  }
